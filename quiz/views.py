...
@login_required(login_url='adminlogin')
def view_submitted_exam_view(request):
    submitted_exams = models.SubmittedExam.objects.all()
    return render(request, 'quiz/view_submitted_exam.html', {'submitted_exams': submitted_exams})
...
def check_exam_status(request, exam_id):
    ...
    if current_time >= exam.start_time and current_time <= exam.end_time:
        exam_status = "Exam is ongoing"
    elif current_time < exam.start_time:
        exam_status = "Exam has not started yet"
    else:
        exam_status = "Exam has ended"
    return render(request, 'quiz/exam_status.html', {'exam_status': exam_status})
